 ...
 */
fun Path.resolveSibling(part: String): Path = parent?.let { Path(it, part) } ?: kotlinx.io.files.Path(part)
 ...
 * See Java `File.resolveSibling`
 */
fun Path.resolveSibling(part: String, vararg parts: String): Path {
    val sibling = resolveSibling(part)
    return sibling.resolve(*parts)
}
